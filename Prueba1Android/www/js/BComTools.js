...
function loadDeviceCSS() {
   ...
   /* if (isIOS() && getOSVersion() >= 7) {
       // parent.document.body.style.marginTop = "20px";
        parent.document.body.style.fontFamily = "helvetica neue";
    }*/
    else if (isIOS()) { parent.document.body.style.fontFamily = "helvetica"; }
}
...
function selectAppropiateResolution() {
    ...
    if (isIOS()) {
		if (minWidth == 1024) {
			...
			needToReload = true;
            console.log('* Ipad Normal');
		} else if (minWidth == 2048) {
			filePath = 'css/retina.css';
			needToReload = true;
            console.log('* Ipad Retina');
		} else {
		    filePath = 'css/normal.css';
			needToReload = true;
            console.log('* Ipad No especificada');
		}
	} else {
        if (currentHeight <= 600) {
            filePath = 'css/1024x600.css';
            needToReload = true;
            console.log('* Android 1024X600');
        } else if (currentHeight <= 800) {
            filePath = 'css/1280x800.css';
            needToReload = true;
            console.log('* Android 1280X800');
        } else if (currentHeight <= 1200) {
            filePath = 'css/1920x1200.css';
            needToReload = true;
            console.log('* Android 1920X1200');
        } else {
            filePath = 'css/1024x600.css';
            needToReload = true;
            console.log('* Android No especificada');
        }
	}
...
function showScreenResolutions() {
    console.log('inner: ' + window.innerWidth + 'x' + window.innerHeight + '\n' + 'outer: ' + window.outerWidth + 'x' + window.outerHeight + '\n' +'current: ' + window.currentWidth + 'x' + window.currentHeight + '\n' + 'avail: ' + screen.availWidth + 'x' + window.availHeight + '\n' + 'normal: ' + screen.width + 'x' + window.height + '\n');
}

function showParentScreenResolutions(context) {
    console.log('inner: ' + context.window.innerWidth + '\n' +'outer: ' + context.window.outerWidth + '\n' + 'current: ' + context.window.currentWidth + '\n' + 'avail: ' + context.screen.availWidth + '\n' + 'normal: ' + context.screen.width + '\n');
}
...
AccountNumberValidator.prototype = {
    ...
    displayKeyInfo: function (eventName) {
        console.log(eventName + ':  keyCode(' + event.keyCode + ') / key(' + key() + ')');
    },
    ...
    pushDigit: function (elementId, keyCode, maxLength) {
        if (this.inputBackup.length == maxLength) {
            console.log('pushDigit: Max length reached.');
            return false;
        } else {
    ...
    popDigit: function (elementId) {
        if (0 == this.inputBackup.length) {
            console.log('popDigit: Min length reached.');
            return false;
        } else {
...
NumericInputValidator.prototype = {
    ...
    isAlpha: function (keyCode) {
        return ((96 < keyCode && keyCode < 123) || (64 < keyCode && keyCode < 91) || (keyCode == 13));
    },
    ...
    hideInputNumber: function (elementId, maxLength, callerEvent) {
        ...
        if ('onkeyup' == callerEvent && this.isReturnKey(keyCode) && this.popDigit(elementId)) {
            ...
            if (keyCode == 13) {
                loginDelegate.botonEntrarLogin();
            } else {
                document.getElementById(elementId).value = this.getMaskedText(elementId);
            }
        } else {
...
/* Valida que solo se ingeresen dÌgitos. */
...
OnlyNumbersValidator.prototype = {
    ...
    validateWithLength: function (source, maxLength) {
        ...
        console.log('length: ' + actualLength);
        console.log('result: ' + (keyCode < 47 || keyCode > 57 || actualLength >= maxLength));
...
function showLoadingLayer() {
    ...
    if (parent.document.getElementById('imgMenu'))
...
function validateRFC(rfc, benef){
    ...
    console.log("validateRFC RFC "+rfc+" benef "+benef+" beneficiario "+beneficiario);
    var resultado = false;
    
    var regexRFC = new RegExp("^[A-Za-z]{4}([0-9]{2})(1[0-2]|0[1-9])([0-3][0-9])([ -]?)([A-Za-z0-9]{3})$");
    resultado = regexRFC.test(rfc);
    console.log("RFC "+rfc+" pasa validación REGEX "+resultado);
    ...
    if (resultado){
        ...
        //console.log("nombre "+nombre+" apellidoPaterno "+apellidoPaterno+" apellidoMaterno "+apellidoMaterno);
        
        
        //console.log("ValidaRFC validaApellidoPaterno "+validaApellidoPaterno+"  validaApellidoMaterno "+validaApellidoMaterno+ " validaNombre "+validaNombre);
        //resultado = validaApellidoPaterno && validaApellidoMaterno && validaNombre;
    }
    console.log("RFC "+rfc+" RESULTADO "+resultado);
...
function toUpperCase(valor) {
    ...
    var valorRFC = valor.value;
    var label = valor.id;
    valorUp = valorRFC.toUpperCase();
    
//    console.log("valorRFC = " + valorRFC + " | valorUp = " + valorUp);
    valor.value = valorUp;

    return valor;
}

// Actualiza el text-indent de los select para centrar el option "Selecciona"
function actualizaTextIndent(objetoHtml, porcentaje) {
    
    // objetoHtml = select desde el que se llama a la función
    // porcentaje = % de text-indent que hay que aplicar para centrar el option "Selecciona"
    
    var textIndent = porcentaje + "%";
    
    if (objetoHtml.value == "-1") {
        $("#"+objetoHtml.id).css({"text-indent": textIndent, "color": "rgb(204,204,204)"});
        
    } else {
        $("#"+objetoHtml.id).css({"text-indent": "0px", "color": "#333"});
    }
    
}
// Importa un pdf desde la url especificada con el nombre sin extencion dado
function guardarPDFFromUrl(nombrePdf, sourceUrl, directorio, successCallB, failCallB){
    parent.guardadoComprobantesDelegate.savePDFWithNameAndFromUrl(nombrePdf, sourceUrl, directorio,successCallB, failCallB);
}



/**
 * Mostrar n últimos dígitos de una cuenta, debe ser tipo String
 *
 * @param numeroCuenta
 * @returns {String}
 */
...
/**
 *  Crea el paginador para los listados de cuentas origen y destino en las pantalas de Drag'n'Drop de transferencias
 */
function cargaPaginador (listado, longitud, mostrar) {
    
    if (mostrar) {
        
        if (listado == '') { // Paginador para transferencias entre mis cuentas
            
            var i = 0;
            
            for (i = 0; i < Math.round(longitud/4); i++) {
                var pagina = i+1;
                
                if (pagina == 1) {
                    $("#paginadorOrigen, #paginadorDestino").append("<li class='numeroPagina selected' data-destino='" + i + "' onclick='cargaPagina(this)' >"+ pagina + "</li>");
                } else if (pagina < 4) {
                    $("#paginadorOrigen, #paginadorDestino").append("<li class='numeroPagina' data-destino='" + i + "' onclick='cargaPagina(this)' >"+ pagina + "</li>");
                }
                
            }
            
            if (longitud > 12) {
                $("#paginadorOrigen, #paginadorDestino").append("<li class='numeroPagina' data-status='activo' data-destino='siguiente' onclick='cargaPagina(this)'>&gt;</li>");
                $("#paginadorOrigen, #paginadorDestino").append("<li class='numeroPagina' data-status='activo' data-destino='fin' onclick='cargaPagina(this)' >&gt;&gt;</li>");
            } else {
                $("#paginadorOrigen .numeroPagina:nth-of-type(1), #paginadorOrigen .numeroPagina:nth-of-type(2), #paginadorOrigen .numeroPagina:nth-of-type(6), #paginadorOrigen .numeroPagina:nth-of-type(7)").css("visibility", "hidden");
                $("#paginadorDestino .numeroPagina:nth-of-type(1), #paginadorDestino .numeroPagina:nth-of-type(2), #paginadorDestino .numeroPagina:nth-of-type(6) #paginadorDestino .numeroPagina:nth-of-type(7)").css("visibility", "hidden");
            }
            
            $("#paginadorOrigen, #paginadorDestino").show();
            
        } else { // Paginador para transferencias BBVA-Bancomer e Interbancarias
            
            if (listado == "origen") {
            
                
                for (var i = 0; i < Math.round(longitud/4); i++) {
                    var pagina = i+1;
                    
                    if (pagina == 1) {
                        $("#paginadorOrigen").append("<li class='numeroPagina selected' data-destino='" + i + "' onclick='cargaPagina(this)' >"+ pagina + "</li>");
                    } else if (pagina < 4) {
                        $("#paginadorOrigen").append("<li class='numeroPagina' data-destino='" + i + "' onclick='cargaPagina(this)' >"+ pagina + "</li>");
                    }
                }
                
                if (longitud > 12) {
                    $("#paginadorOrigen").append("<li class='numeroPagina' data-status='activo' data-destino='siguiente' onclick='cargaPagina(this)'>&gt;</li>");
                    $("#paginadorOrigen").append("<li class='numeroPagina' data-status='activo' data-destino='fin' onclick='cargaPagina(this)' >&gt;&gt;</li>");
                } else {
                    $("#paginadorOrigen .numeroPagina:nth-of-type(1), #paginadorOrigen .numeroPagina:nth-of-type(2), #paginadorOrigen .numeroPagina:nth-of-type(6), #paginadorOrigen .numeroPagina:nth-of-type(7)").css("visibility", "hidden");
                }
                
                $("#paginadorOrigen").show();
                
            } else if (listado == "destino") {
                
                for (var i = 0; i < Math.round(longitud/4); i++) {
                    var pagina = i+1;
                    
                    if (pagina == 1) {
                        $("#paginadorDestino").append("<li class='numeroPagina selected' data-destino='" + i + "' onclick='cargaPagina(this)' >"+ pagina + "</li>");
                    } else if (pagina < 4) {
                        $("#paginadorDestino").append("<li class='numeroPagina' data-destino='" + i + "' onclick='cargaPagina(this)' >"+ pagina + "</li>");
                    }
                }
                
                if (longitud > 12) {
                    $("#paginadorDestino").append("<li class='numeroPagina' data-status='activo' data-destino='siguiente' onclick='cargaPagina(this)'>&gt;</li>");
                    $("#paginadorDestino").append("<li class='numeroPagina' data-status='activo' data-destino='fin' onclick='cargaPagina(this)' >&gt;&gt;</li>");
                } else {
                    $("#paginadorDestino .numeroPagina:nth-of-type(1), #paginadorDestino .numeroPagina:nth-of-type(2), #paginadorDestino .numeroPagina:nth-of-type(6), #paginadorDestino .numeroPagina:nth-of-type(7)").css("visibility", "hidden");
                }
                
                $("#paginadorDestino").show();
