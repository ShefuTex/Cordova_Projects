...
(function(){
	...
	function _stxAdvanced_js(_exports) {
		...
		STX.fetchEquationChart=function(params,cb){
			...
			var formEq=formatEquation(params.symbol);
		...
		STX.computeEquationChart=function(equation, map){
			...
			var count=0;
			for(var sym in map){
				var r=new RegExp("\\["+sym.replace("^","\\\^")+"\\]","g");
				...
				if(aligned==1){
					...
					if(priceRelative){
						...
						*/
						...
						m.Adj_Close=Math.round(m.Adj_Close*10000)/10000;
		...
		STX.calculateKagi=function(stx, dataSet, reversal){
			...
			if(!reversal){
				...
				if(STX.Market.ChartIQ.isForexSymbol(stx.chart.symbol)) reversal/=4;
			}else{
			...
			function processMove(q, b, isFinal){
				while(1){
					...
					if(typeof(targetPrice)=="undefined" || typeof(currentPrice)=="undefined"){
						console.log("Uh oh undefined in createRangeBars:processMove");
						return;
					}
		...
		STX.calculatePointFigure=function(stx, dataSet, pandf){
			...
			if(!box) {
				...
				if(STX.Market.ChartIQ.isForexSymbol(stx.chart.symbol)) {
					if(lastPrice){
...
*/
		...
		STX.Studies.calculatePivotPoints=function(stx, sd){
			...
			var isForex=STX.Market.ChartIQ.isForexSymbol(stx.chart.symbol);
			var isMetal=STX.Market.ChartIQ.isForexMetal(stx.chart.symbol);
		...
		STX.Studies.calculatePriceRelative=function(stx, sd){
			...
			var cSym=sd.inputs["Comparison Symbol"].split(":")[0].toUpperCase(); //do not allow : in symbol
		...
		*/
		...
		/**
		 * Sample `placementFuncion` override used to draw markers above a particular candle, bar or line value.
		 *
