	...
	function _stxLibrary_js(_stxThirdParty,_exports) {
		...
		STX.QuoteFeed.BarChart.prototype.fetch=function(params, cb){
			...
			STX.postAjax(url, null, function(status, res){
				...
				else if(STX.Market.ChartIQ.isForexSymbol(params.symbol)) attrExch="REAL-TIME";
		...
		STX.QuoteFeed.Xignite.prototype.fetch=function(params, cb){
			...
			var isForex=STX.Market.ChartIQ.isForexSymbol(symbol);
			var isFuture=STX.Market.ChartIQ.isFuturesSymbol(symbol);
			...
			if(!params.symbol){
				...
				if(!params.update && isDaily){
					if(STX.Market.ChartIQ.isForexMetal(symbol)){
					...
					if(STX.Market.ChartIQ.isForexMetal(symbol,true)){
					...
					if(isDaily && !params.loadMore) {
						...
						if(!params.update){
							...
							if(!results.length || STX.yyyymmddhhmm(new Date(myDate)).substr(0,8)!=STX.yyyymmddhhmm(results[results.length-1].DT).substr(0,8)){
								if(!expiredFuture && !STX.Market.ChartIQ.isForexMetal(symbol,true) && !params.noUpdate){
