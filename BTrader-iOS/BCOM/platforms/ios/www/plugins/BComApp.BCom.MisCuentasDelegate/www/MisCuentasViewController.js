...
//    var creditCardsChartColors = [ '#0170c3', '#fabe30', '#e81060' ];
    ...
    var lineColors = [ '#6eafe1', '#fabe30', '#8bc633', '#f28b25', '#86d3f1', '#66c1e2' ];
    var lineColor;
    ...
    var accountsChartSelectedLegendColors = [ '#8c8c8c', '#6eafe1', '#fabe30', '#8bc633', '#f28b25', '#86d3f1', '#66c1e2' ];
...
/* ______________________________________________________________ */
    ...
    MisCuentasViewController.prototype.loadSaldosChart= function (containerId) {
           ...
           var saldosPlot = new Highcharts.Chart({
                         ...
                         borderColor: null,
                         formatter: function () {
                         return ('<div class="saldosCartTooltipTitle" style="color:' + this.series.color + ';">' + misCuentasViewController.formatForCurrency(this.y, true, $) + '</div>' +
    ...
    MisCuentasViewController.prototype.formatForCurrency= function (amount, showDecimals, showCurrencySymbol) {
           console.log(amount);
           var result = (amount + '').replace(/.(?=(?:[0-9]{3})+\b)/g, '$&,');
    ...
    MisCuentasViewController.prototype.onAccountSelectionChanged= function (index, isCreditCards) {
               console.log("Indice selected----------  "+index+" ----- \n");
    ...
    MisCuentasViewController.prototype.pintarGraficaCuentasDolares= function () {
           ...
           for ( var i = 0; i < cuentasUSD.length; i++) {
                if ((typeof cuentasUSD[i].alias == "undefined") || (cuentasUSD[i].alias == null) || (cuentasUSD[i].alias == "")) {
                    cuentasUSD[i].alias = cuentasUSD[i].numero.substring(cuentasUSD[i].numero.length-10, cuentasUSD[i].numero.length);
                }
    ...
    MisCuentasViewController.prototype.accountsLinkClicked= function () {
           ...
           accountsChartSelectedLegendColors = [ '#8c8c8c', '#006ec1', '#fabe30',
    ...
    MisCuentasViewController.prototype.creditCardsLinkclicked= function () {
           ...
           $('#accountTypeSection a:nth-child(5)').addClass('selec-nav');
           $('#accountTypeSection a:first-child').removeClass('selec-nav');
           $('#accountTypeSection a:nth-child(3)').removeClass('selec-nav');
           ...
           $(document).ready(function () {
                ...
                if ((typeof accountListSelected != "undefined")
                    ...
                    if (typeof id == "undefined")
                        ...
                        var datosAplic = new DatosAplicativos(sessionStorage.username,
                                                 pg.posicionGlobal.acceso_usr, id, periodo);
                        var peticion = new ConsultaMovimientosPeticion("imd_movimientos_tdc_pr", "imd_movimientos_tdc_op", "movimientos",datosAplic);
           $(document).ready(function () {
...
//    }
    ...
    MisCuentasViewController.prototype.moverBarrasIzq= function () {
           ...
           if (ultimaBarraMostrada == 6) {
                $("#flechaIzqON").attr("id", "flechaIzqOFF");
                $("#flechaIzqOFF").attr("onclick", "");
           } else if ((accountListSelected.length - 1) == ultimaBarraMostrada) {
                $("#flechDerechaOFF").attr("id", "flechDerechaON");
                $("#flechDerechaON").attr("onclick", "misCuentasViewController.moverBarrasDrch()");
           }
    ...
    MisCuentasViewController.prototype.inicializarValoresEjeX= function () {
           ...
           if (!banderaTDC) {
                for (i = 0; i < accountVisibles.length; i++) {
                    accountVisiblesXAxis.push(accountVisibles[i].name.toLowerCase());
                    accountVisiblesBalances.push(accountVisibles[i].data);
                    chartDatas.push(accountVisibles[i].data);
                }
           } else {
                for (i = 0; i < accountVisibles.length; i++) {
                    accountVisiblesXAxis.push(accountVisibles[i].alias.toLowerCase());
               
                    // En Android han levantado una incidencia en la que dicen que las barras de la gr‡fica de tarjeta de crŽdito est‡n
                    // representando los saldos intercambiados.
                    // Cambiamos "accountVisibles[i].saldocorte" por "accountVisibles[i].saldofecha" para implementar el mismo comportamiento
                ...
                chartDatas.push({name: 'Saldo a fecha', data: creditCardVisibleDateBalance}, {name: 'Saldo al corte', data: creditCardVisibleCutBalance, color: creditCardsChartColors[2]});
           }
...
console.log("inicializarGastos.id = " + id);
           ...
           window.setTimeout(function () {
            hideLoadingLayer(); loading = false;
           var gastos = ret;
...
//                        }
    ...
    MisCuentasViewController.prototype.generateData= function () {
           console.log("Generando datos GRçFICA SALDOS ");
               console.log("banderaMXP =", banderaMXP);
               console.log("banderaUSD =", banderaUSD);
               console.log("banderaTDC =", banderaTDC);
           saldosChartData = [];
           ...
           if ((typeof listaMov != "undefined") && (listaMov != null)
               && (listaMov.length > 0)) {
                ...
                if (!banderaTDC) {
                    ...
                    var importe = parseFloat(listaMov[0].saldo);
//                    var abonoMov = parseFloat(0);
//                    var cargoMov = parseFloat(0);
//                    var saldoMov = parseFloat(0);
//               
//                    var diaAux = parseInt(listaMov[0].fecha.substring(0, 2));
//                    var auxDia = diaAux;
//               
//                   for (var j = 0; j < listaMov.length; j++) {
//                   
//                       if (auxDia == diaAux) {
//                           saldoMov = parseFloat(listaMov[j].saldo);
//                           if ((typeof listaMov[j].abono != "undefined")
//                               && (listaMov[j].abono != "")
//                               && (listaMov[j].abono != null)) {
//                                abonoMov = parseFloat(listaMov[j].abono);
//                           } else {
//                                abonoMov = parseFloat(0);
//                           }
//                           if ((typeof listaMov[j].cargo != "undefined")
//                               && (listaMov[j].cargo != "")
//                               && (listaMov[j].cargo != null)) {
//                                cargoMov = parseFloat(listaMov[j].cargo);
//                           } else {
//                                cargoMov = parseFloat(0);
//                           }
//               
//                           console.log("j =", j);
//                           console.log("auxDia =", auxDia);
//                           console.log("saldoMov =", saldoMov);
//                           console.log("abono =", abonoMov);
//                           console.log("cargo =", cargoMov);
//                           console.log("--------------------------------");
//               
//                       }
//               
//                       auxDia = parseInt(listaMov[j].fecha.substring(0, 2));
//                   }
//                   console.log("importe = " + saldoMov + " + " + cargoMov + " - " + abonoMov);
//                   console.log("--------------------------------");
//                   importe = saldoMov + cargoMov - abonoMov;
//               
                   console.log("importe inicial TDD =", importe);
                } else {
                    if (periodo == 0) {
                        var importe = parseFloat(misCuentasViewController.recuperaSaldoTDC(sessionStorage.cuentaSeleccionadaId));
                    } else if (periodo == 1) {
...
//                    misCuentasViewController.inicializarGastos(accountVisibles[index].id);
    ...
    MisCuentasViewController.prototype.ConsultaMovimientosCtasCheques= function (peticion) {
           ...
           if ((typeof peticion != "undefined") && (peticion != null)) {
                if (banderaMXP) {
                    execute(function (jsonResponse) { misCuentasViewController.returnRespuesta(jsonResponse); },
                            function (errorMessage) { hideLoadingLayer(); misCuentasViewController.returnRespuesta(errorMessage);/*compraInvertirDelegate.muestraAlert("Aviso\n", JSON.parse(errorMessage).error.mensaje, "Aceptar");*/  },
                            "MisCuentasDelegate",
                            "recuperaCuentasTDDPesos",
                            [peticion]
                            );
                } else if (banderaUSD) {
                    execute(function (jsonResponse) { hideLoadingLayer(); misCuentasViewController.returnRespuesta(jsonResponse); },
                            function (errorMessage) { hideLoadingLayer(); misCuentasViewController.returnRespuesta(errorMessage); },
                            "MisCuentasDelegate",
    ...
    MisCuentasViewController.prototype.returnRespuesta= function (jsonResponse) {
           ...
           this.respuesta = JSON.parse(jsonResponse);
           ret = [];
           var list = [];
           tablaDatos = [];
          tableContent = "";
               
        if (typeof this.respuesta.respuesta != "undefined"){
               this.respuesta=this.respuesta.respuesta;
      
            
               if (banderaTDC){
               tablaDatos = list = misCuentasViewController.ordenaPorFecha(this.respuesta.movimientos.ListaMovimientosTDC);
...
//                            console.log("tablaDatos[" + index + "] = " + tablaDatos[index]);
                            if ((typeof tablaDatos[index].abono != "undefined")
                                    ...
                                    + "</span></th>" + "<td>" + misCuentasViewController.applyImpMask(tablaDatos[index].abono)
                                    + "</td></tr>";
                                    ...
                                    + "<span>" + misCuentasViewController.parseDate(tablaDatos[index].fecha)
                                    + "</span></th><td>- "
                                    + misCuentasViewController.applyImpMask(tablaDatos[index].cargo) + "</td></tr>";
                            }
                    ...
                    for (index = tablaDatos.length-1; index >= 0; index--) {
                        if ((typeof tablaDatos[index].importe_operacion != "undefined")
                            ...
                            if (parseFloat(tablaDatos[index].importe_operacion) > 0) {
                                ...
                                + "<span>" + misCuentasViewController.parseDate(tablaDatos[index].fecha)
                                + "</span></th>" + "<td>"
                                + misCuentasViewController.applyImpMask(tablaDatos[index].importe_operacion)
                                + "</td></tr>";
                            } else {
                                tableContent += "<tr><th>" + tablaDatos[index].concepto
                                + "<span>" + misCuentasViewController.parseDate(tablaDatos[index].fecha)
                                + "</span></th>" + "<td style='color:#e81060'>- $ "
                                +misCuentasViewController.applyImpMask(Math.abs(tablaDatos[index].importe_operacion)).substring(1)
                                + "</td></tr>";
                            }
                        }
...
//           console.log("tablaActiva = " + tablaActiva);
      ...
      }else if (this.respuesta!=undefined && this.respuesta.error!=undefined) {
           // compraInvertirDelegate.muestraAlert("Aviso\n", this.respuesta.error.mensaje, "Aceptar");
            ...
            //   compraInvertirDelegate.muestraAlert("Aviso\n", this.respuesta.errorArq.mensaje, "Aceptar");
   ...
   //console.log("REspuesta en MisCuentasViewController.MuestraPantallaREsultado: "+jsonResponse);
    ...
    console.log("response en MisCuentasViewController.MuestraPantallaREsultado: "+response);
            
    //response=undefined;
               
    if (typeof responseRespuesta != "undefined"){
               
    var folio = JSON.parse(jsonResponse).respuesta.folioInternet;
               
               
    var mensaje = JSON.parse(jsonResponse).respuesta.listaMensajes;
...
//   document.location = 'transferirMisCuentasResultado.html?origen='+QueryString.origen+'&destino='+QueryString.destino+'&importe='+QueryString.importe+'&fecha='+QueryString.fecha+'&folio='+folio+'&comision=12.223';
     ...
     } // de     if (response!=undefined){
      else{
         
               if (typeof response.error !="undefined"){
               
             //  compraInvertirDelegate.muestraAlert("Aviso\n", response.error.mensaje,"Aceptar");
               
               
               }else {
               
             //  compraInvertirDelegate.muestraAlert("Aviso\n", response.errorARQ.msg,"Aceptar");
   ...
   MisCuentasViewController.prototype.pagoMinimoNoInteres = function (peticion) {
               execute(function (jsonResponse) {
                       console.log("pagoMinimo " + jsonResponse);
                       sessionStorage.pagoMinimo = jsonResponse; },
                       function (errorMessage) { /*compraInvertirDelegate.muestraAlert("Aviso\n", JSON.parse(errorMessage).error.mensaje, "Aceptar");*/ },
                       "TransferirDelegate",
                       "pagoMinimoNoInteres",
                       [peticion]
                       );
   ...
   MisCuentasViewController.prototype.solicitaComision = function (peticion) {
               execute(function (jsonResponse) { sessionStorage.comision = jsonResponse; },
                       function (errorMessage) { hideLoadingLayer(); /*compraInvertirDelegate.muestraAlert("Aviso\n", JSON.parse(errorMessage).error.mensaje, "Aceptar");*/  },
                       "TransferirDelegate",
                       "solicitaComision",
                       [peticion]
                       )
    ...
    MisCuentasViewController.prototype.recuperaSaldoTDC = function (id) {
               
        var saldo;
            
//        var cuentasTDC = pg.posicionGlobal.asuntos.lista_tarjetascredito;
               
        for (var i = 0; i < cuentasTDC.length-1; i++) {
            if (cuentasTDC[i].id == id) {
               saldo = cuentasTDC[i].saldofecha;
            }
        }
