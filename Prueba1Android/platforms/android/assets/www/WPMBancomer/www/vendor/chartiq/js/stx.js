...
/**
 * The market class is used to manage tading hours for the different exchanges.
 * Only one market object should be created and attached to the chat at any given time. See {@link STXChart#setMarket} and {@link STXChart#setMarketFactory} for instructions on how to assing a market class to a chart.
 *
 * See the [NYSE market definition]{@link STX.Market.NYSE}  for details about how to define a market.
 ...
 * @param {Object} [parms.market_definition] A json object that contains the rules for some market. If not defined default market is always open.
 * @param {String} [parms.market_definition.tz_lib] If the market needs to be aware of different time zone and daylight savings time (probably does) this property is set to the constructor of a time zone lib. For example to use timezone.js tz_lib is set to timezonejs.Date. This allows the market construct Date objects that will produce the correct offset for various time zones that are daylight savings time aware.
 * @param {Boolean} [parms.market_definition.convertOnDaily] By default, daily charts are not converted for timezone. Set this to true to convert for daily charts (either in the market_definition or the Market class instance itself)
 *
...
STX.Market = function(parms) {
	...
	if (parms && parms.market_definition) {
		this.market_def = parms.market_definition;
		if (this.market_def.rules) {
			this.rules = this.market_def.rules;
		}
		if (parms.market_definition.market_tz) {
			this.market_tz = parms.market_definition.market_tz;
		}
		if (parms.market_definition.convertOnDaily) {
			this.convertOnDaily = parms.market_definition.convertOnDaily;
		}
		if (parms.tz_lib) {
			this.tz_lib = parms.tz_lib;
		}
		if (typeof parms.market_definition.hour_aligned) {
			this.hour_aligned = parms.market_definition.hour_aligned;
		}
 ...
 */
STX.Market.ChartIQ.isForeignSymbol=function(symbol){
 ...
 */
STX.Market.ChartIQ.isFuturesSymbol=function(symbol){
	if(!symbol) return false;
 ...
 */
STX.Market.ChartIQ.isForexSymbol=function(symbol){
	if(!symbol) return false;
	if(STX.Market.ChartIQ.isForeignSymbol(symbol)) return false;
	if(STX.Market.ChartIQ.isFuturesSymbol(symbol)) return false;
 ...
 */
STX.Market.ChartIQ.isForexMetal=function(symbol,inverse){
	if(!symbol) return false;
	if(!STX.Market.ChartIQ.isForexSymbol(symbol)) return false;
...
STX.Market.ChartIQ.isForexFuturesSymbol=function(symbol){
	if(STX.Market.ChartIQ.isForexSymbol(symbol)) return true;
	if(STX.Market.ChartIQ.isFuturesSymbol(symbol)) return true;
	return false;
};

/**
 * Example Market factory that uses ChartIQ's symbology
 * @param  {Object} symbolObject Symbol object of form accepted by {STXChart#newChart}
 * @return {Object}              A market definition
 */
STX.Market.ChartIQ.factory=function(symbolObject){
	var symbol=symbolObject.symbol;
	if(STX.Market.ChartIQ.isForeignSymbol(symbol)) return null; // 24 hour market definition
	if(STX.Market.ChartIQ.isFuturesSymbol(symbol)) return STX.Market.GLOBEX;
	if(STX.Market.ChartIQ.isForexMetal(symbol)) return STX.Market.METALS;
	if(STX.Market.ChartIQ.isForexSymbol(symbol)) return STX.Market.FOREX;
	return STX.Market.NYSE;
};

/**
 * Return the time in the market zone
 * @memberof STX.Market
 */
 ...
 * This function is daylight savings time aware if the market is using
 * timezone js or other similar libray.
...
/**
 * Factory function to get iterators for this Market. An iterator instance can
 * go forward or backward in time an arbitrary amount. However, the internal
 * state cannot be changed once it is constructed. A new iterator should be
 ...
 *
 * @param {Object} parms Parameters used to initalize the Market object.
 * @param {Object} parms.interval This is a variable found in the kernal it
 * might be an integer or a string. One of the main things this function does
 * is deal with the difference in this variable.
 * @param {Integer} parms.periodicity This is a variable found in the kernel
 * The internal clock code refers to this as the multiple. That is if the
 * chart is a 5 minute chart and multiple/periodicity is 3 then each call to
 * next or previous will move at least 15 minutes. Default is 1 if not
 * specified.
 * @param {Date} parms.begin The date to set as the start date for this
 * iterator instance.
 * @param {Boolean} parms.return_data_zone Optional default is false. By default
 * when the data is intra day data the iterator will return dates shifted to
 * the display zone. In some cases the kernel needs the date to be in data zone
 * set this argument to true in that case.
 * @param {String} parms.timeUnit Optional. If set to 'milliseconds' or 'seconds'
 * this will override parms.interval. It is done this way to maintain backwards
 * compatibiliy with variables in the kernel.
 * @return {Object} A new iterator.
 * @memberof STX.Market
 * @example
 * var iter = params.stx.chart.market.newIterator(
			{
				'begin': now,
...
STX.Market.prototype.newIterator = function(parms) {
	...
	if (!typeof parms.return_data_zone) {
		parms.return_data_zone = false;
	}
	if (!parms.begin) {
...
/**
 * Set the data zone for this market. Note that this will not take effect on
 * existing iterators. All iterators should be discarded after this call and
 * reconstructed.
 * @param {String} datatz_str A time zone string. IE America/New_York
 * @memberof STX.Market
 */
STX.Market.prototype.setDataZone = function(datatz_str) {
	if (datatz_str) {
		this.data_tz = datatz_str;
	}
};

/**
 * Set the display zone for this market. Note that this will not take effect on
 * existing iterators. All iterators should be discarded after this call and
 * reconstructed.
 * @param {String} datatz_str A time zone string. IE America/New_York
 * @memberof STX.Market
 */
STX.Market.prototype.setDisplayZone = function(displaytz_str) {
	if (displaytz_str) {
		this.display_tz = displaytz_str;
	}
};
 ...
 * Note don't change the name of this or it will mess up the unit tests.
 */
...
/**
 * DO NOT USE.
 * Use STX.Market.newIterator(...) instead.
 */
STX.Market.Iterator = function(parms) {
	...
	this.daily = this.clock.intra_day;
 ...
 * market open, close, and holidays. Note that the begin date is set when this
 * instance of the iterator is created and one should NOT call previous or next
 * when needing future ticks.
 * @param {Date} parms.end An end date.
 * @param {Integer} parms.sample_size Optional. Default is 25. Number of
 * milliseconds used as a maximum time to count ticks. If sample size is
 ...
 * is.
 * @param {Integer} parms.sample_rate Optional. Default is 1000.
 ...
 * @param {Integer} skip Optional. Default 1. Jump forward skip * periodicity at once.
 * @return {Date} Next date. Depending on settings may be display, datazone or market.
 * @alias next
 * @memberof! STX.Market#
 * @example
 * now = iter.next();
...
STX.Market.Iterator.prototype.next = function(skip) {
	...
	if(!this.daily || this.market.convertOnDaily){
 ...
 * @return {Date} Previous date. Depending on settings may be display, datazone or market.
 * @alias previous
 * @memberof! STX.Market#
 * @example
 * now = iter.previous();
...
/**
 * Create an array of minute boundaries on the last time segment that was
 * matched.
 */
_ClockP._alignMinutes = function() {
	...
	var next_minute = open_minute;
	var periods = [];
	var post_periods = [open_minute];
	var pre_periods = [];
	...
	while (true) {
		...
		post_periods.push(next_minute);
	}

	//wrap around
	while (next_minute < open_minute) {
		pre_periods.push(next_minute);
		next_minute += this.multiple;
	}
	periods = periods.concat(pre_periods);
	periods = periods.concat(post_periods);
	return periods;
};
...
_ClockP._setStart = function(date) {
	var millis = this.market._tzDifferenceMillis(
		date/*, this.market.data_tz, this.market.market_tz*/);
...
_ClockP._minuteImpl = function() {
	...
	current_minute = this._alignToBoundary(periods, current_minute);
	current_date.setMinutes(current_minute);
	...
	if (this._windMaybe(this.market._wasOpenIntraDay, this._tickTock)) {
		if (this.forward) {
			...
			current_date = new Date(this.ctime);
			current_date.setMinutes(periods[periods.length - 1]);
			this.ctime = current_date.getTime();
		}
	}
	return this._date();
...
//*** end copy paste of Market
		...
		*/
		...
		STX.TimeZoneWidget.timeZoneMap={
				...
				"(GMT+03:00) Minsk, Moscow, Volgograd":"Europe/Moscow",
				...
				"(GMT+07:00) Barnaul, Krasnoyarsk":"Asia/Krasnoyarsk",
				...
				"(GMT+10:00) Vladivostok, Magadan":"Asia/Vladivostok",
		...
		STXChart.version=["Version 2016-04-12"];
		...
		function STXChart(config){
		     ...
		     * @memberOf  STXChart.prototype
		     * @since TBD The line will be extended to the end of the chart (full candle width) instead of the candle border, even when using yaxisLabelStyle "roundRectArrow"
		     */
		...
		STX.QuoteFeed.Demo.prototype.generateDaily=function(params, cb){
			...
			if(symbol.charAt(0)!="^" && STX.Market.ChartIQ(symbol)) symbol="^"+symbol;
		...
		STX.QuoteFeed.Demo.prototype.generateIntraday=function(params, cb){
			if(params.stx.marketFactory)
				params.stx.setMarket(params.stx.marketFactory(params), params.stx.chart);
		...
		STX.QuoteFeed.ChartIQEOD.prototype.fetch=function(params, cb){
			...
			var symbol=params.symbol.toUpperCase();
			if(symbol.charAt(0)!="^" && STX.Market.ChartIQ.isForexSymbol(symbol)) symbol="^"+symbol;
