...
BaseViewController.prototype = {
    ...
    initPage: function () {
        ...
        //$('.lateral-trader').css('display', this.isWebTraderEnabled() ? 'block' : 'none');
    ...
    validaLocation: function (seccion, opcion, paso) {
        var posG = false;
        //showLoadingLayer();
        if (seccion == 'isWebTrader') { 
        	mostrarMBComWebTrader(opcion, paso);
        } else {
            posG = true;
    ...
    asignaDatos: function () {
        ...
        if (jsonObject.posicionGlobal.nombre_usr) {
            //document.getElementById("lblNombreCliente").innerHTML = jsonObject.posicionGlobal.nombre_usr;
        }

        if (jsonObject.posicionGlobal.fecha_sesionAnterior || jsonObject.posicionGlobal.hora_sesionAnterior) {
           // var date = formatDate(jsonObject.posicionGlobal.fecha_sesionAnterior);
           // var time = formatTime(jsonObject.posicionGlobal.hora_sesionAnterior);

           // document.getElementById("lblUltimaSesion").innerHTML = 'Última sesión ' + date + ' ' + time;
        }
    },
    ...
    isWebTraderEnabled: function () {
        var posG = JSON.parse(sessionStorage.posicionGlobal);
