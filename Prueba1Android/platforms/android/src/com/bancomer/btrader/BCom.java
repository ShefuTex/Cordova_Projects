...
public class BCom extends CordovaActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
    	...
    	if(null != getIntent() && null != getIntent().getExtras() && getIntent().getExtras().containsKey(EXIT_EXTRA_KEY)) {
			...
			Utilerias.applicationContext = this.getApplicationContext();
	...
	public void showAlertInicio(final CallbackContext cordovaCallbackContext, final String title, final String message, final String positiveButtonText) {
	   System.out.println("showAlertInicio 1- BCom.java");
		showAlertInicio(cordovaCallbackContext, title, message, positiveButtonText, null);
    }
	
	public void showAlertInicio(final CallbackContext cordovaCallbackContext, final String title, final String message, final String positiveButtonText, final String negativeButtonText) {
		 System.out.println("showAlertInicio 2- BCom.java");
		this.runOnUiThread(new Runnable() {	
			@Override
			public void run() {
				AlertDialog.Builder builder = new Builder(BCom.getInstance());
...
//FIN  Mensaje inicio de sesion
	
	
	public void showInformationAlert(final String message) {
		System.out.println("showInformationAlert_mario:   "+ message );
		AlertDialog.Builder builder = new Builder(BCom.getInstance());
		String mensaje = "";
